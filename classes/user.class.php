<?php
	class User extends DB{
		...
		// legacy cart-count helpers kept for compatibility
		// the current implementation stores cart data in the `cart`
		// table and in the user session. the users table does not
		// contain a ``cart`` column (and never has in production), so
		// these methods are intentionally no‑ops to avoid SQL errors.
		...
		public function insert_user($username, $email, $password, $repassword, $admin, $role = 'user'){
			$conn = $this->connect();
			$errors = array();
			...
			if(empty($errors)){
				$sql = "INSERT INTO users SET username=?, email=?, password=?, admin=?, role=?;";
				$stmt = $conn->prepare($sql);
				$password = password_hash($password, PASSWORD_DEFAULT);
				$stmt->bind_param("sssis", $username, $email, $password, $admin, $role);
				$stmt->execute();
			}
